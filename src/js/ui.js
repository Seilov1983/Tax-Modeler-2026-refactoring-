...
export function showYearEndWizard() {
  ...
  overlay.style.cssText = "position:fixed; top:0; left:0; width:100vw; height:100vh; background:rgba(224, 229, 236, 0.85); backdrop-filter:blur(8px); display:flex; align-items:center; justify-content:center; z-index:9999;";

  const isClosed = project.periods?.closedYears?.includes(year);
  if (isClosed) {
      overlay.innerHTML = `
          <div style="background: #e0e5ec; padding: 30px; border-radius: 16px; width: 450px; text-align: center; box-shadow: 12px 12px 24px rgb(163,177,198,0.7), -12px -12px 24px rgba(255,255,255, 0.6);">
              <h2 style="color: #e74c3c; margin-top:0;">Период ${year} закрыт 🔒</h2>
              <p style="color: #555;">Внесение изменений, проводка платежей и генерация налогов за этот год заблокированы для защиты Audit Log.</p>
              <div style="display: flex; gap: 12px; justify-content: center; margin-top: 25px;">
                  ...
                  <button class="btn" id="btnWzUnlock" style="background: #e74c3c; color: white; border:none; padding:10px 20px;">🔓 Открыть период</button>
              </div>
          </div>
      `;
  ...
  let netVatText = netVat > 0 ? `К уплате: <b style="color:#e74c3c;">${formatMoney(netVat)} KZT</b>` : (netVat < 0 ? `Переплата: <b style="color:#2ecc71;">${formatMoney(Math.abs(netVat))} KZT</b>` : `<b>0 KZT</b>`);
  ...
  overlay.innerHTML = `
      <div style="background: var(--bg, #e0e5ec); padding: 30px; border-radius: 16px; width: 680px; max-width: 95%; max-height: 90vh; overflow-y: auto; box-shadow: 12px 12px 24px rgb(163,177,198,0.7), -12px -12px 24px rgba(255,255,255, 0.6);">
          
          <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 5px;">
              <h2 style="margin:0; color: #333; font-size: 1.5em;">Закрытие ${year} года</h2>
              <div class="pill ok" style="font-size: 14px; padding: 6px 12px;">Group ETR: ${etr}%</div>
          </div>
          <div style="font-size: 0.95em; color: #666; margin-bottom: 25px;">Математическая модель рассчитана на базе ${executedFlows.length} транзакций.</div>

          <div class="item" style="margin-bottom: 15px; cursor: default;">
              <h4 style="margin: 0 0 8px 0; color: var(--accent, #4F8CFF);">1. Перенос убытков (Loss Carryforward)</h4>
              <p style="margin: 0 0 10px 0; font-size: 0.9em; color: #555;">Доступный убыток: <b style="color:#e74c3c;">- ${formatMoney(totalLosses)} KZT</b></p>
          ...
          <div class="item" style="margin-bottom: 15px; cursor: default;">
              <h4 style="margin: 0 0 8px 0; color: var(--accent, #4F8CFF);">2. Взаимозачет НДС (VAT Netting)</h4>
              <p style="margin: 0 0 10px 0; font-size: 0.9em; color: #555;">Сальдо: ${netVatText}</p>
              ${netVat > 0 ? `
              <label style="font-size: 0.9em; display:flex; align-items:center; gap:10px; cursor: pointer; font-weight: 500;">
                  <input type="checkbox" checked id="wz_vat" style="width:16px; height:16px;"> Сгенерировать единый налоговый платеж
              </label>` : '<div style="font-size: 0.85em; color: #888;">Действия не требуются.</div>'}
          </div>

          <div style="display: flex; gap: 12px; justify-content: flex-end; margin-top:25px;">
              <button class="btn secondary" id="btnWzCancel">Отмена</button>
              <button class="btn" id="btnWzApply" style="background: #a855f7; color: white;">Применить и Закрыть год</button>
          </div>
      </div>
  `;
  document.body.appendChild(overlay);
